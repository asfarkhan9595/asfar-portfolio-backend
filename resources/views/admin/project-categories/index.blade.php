...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Project Categories</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Organize and manage project categories across your portfolio.</p>
        </div>
        <button onclick="document.getElementById('addProjectCatModal').classList.remove('hidden')" type="button" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors shadow-lg shadow-indigo-500/20">
            <i data-lucide="plus" class="w-4 h-4 mr-2"></i> Add Project Category
        </button>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 mb-6 shadow-sm">
        <form method="GET" action="{{ route('admin.project-categories.index') }}" class="flex flex-col sm:flex-row gap-4 items-center justify-between">
            <!-- Search Input -->
            <div class="relative w-full sm:w-80">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400">
                    <i data-lucide="search" class="w-4 h-4"></i>
                </div>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search categories..." class="w-full pl-9 pr-3.5 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500">
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3 w-full sm:w-auto justify-end">
                ...
                @if(request('search') || request('status') || request('sort') || request('per_page'))
                    <a href="{{ route('admin.project-categories.index') }}" class="p-2 text-xs font-medium text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white bg-gray-100 dark:bg-gray-700 rounded-lg transition-colors whitespace-nowrap" title="Clear Filters">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                        ...
                        <td class="px-6 py-4 text-sm relative">
                            @if($item->projects_count > 0)
                                <button type="button" onclick="toggleCatPopover({{ $item->id }})" class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-indigo-100 text-indigo-800 dark:bg-indigo-500/10 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20 hover:bg-indigo-200 dark:hover:bg-indigo-500/20 transition-colors">
                                    <i data-lucide="briefcase" class="w-3.5 h-3.5"></i>
                                    {{ $item->projects_count }} {{ Str::plural('project', $item->projects_count) }}
                                </button>

                                <!-- Linked Projects Popover -->
                                <div id="cat-popover-{{ $item->id }}" class="cat-popover hidden absolute left-6 top-12 z-20 w-64 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 p-3">
                                    <div class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2 border-b border-gray-100 dark:border-gray-700 pb-1 flex justify-between items-center">
                                        ...
                                        <button type="button" onclick="toggleCatPopover({{ $item->id }})" class="text-gray-400 hover:text-gray-600">&times;</button>
                                    </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <form action="{{ route('admin.project-categories.toggle-active', $item->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('PATCH')
                        ...
                        <td class="px-6 py-4 text-right text-sm font-medium space-x-1">
                            <button type="button" onclick="document.getElementById('viewProjectCatModal-{{ $item->id }}').classList.remove('hidden')" class="p-1.5 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-md transition-colors" title="View Category Details">
                                <i data-lucide="eye" class="w-4 h-4"></i>
                            </button>
                            <button type="button" onclick="document.getElementById('editProjectCatModal-{{ $item->id }}').classList.remove('hidden')" class="p-1.5 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-md transition-colors" title="Edit">
                                <i data-lucide="edit-2" class="w-4 h-4"></i>
                            </button>
                            <form action="{{ route('admin.project-categories.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Delete this category?');">
                                @csrf
                                @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                            <div class="flex flex-col items-center justify-center">
                                <i data-lucide="tags" class="w-10 h-10 text-gray-400 mb-2"></i>
                                <p class="text-base font-medium">No project categories found</p>
    ...
    <div id="addProjectCatModal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
        <!-- Backdrop -->
        <div onclick="document.getElementById('addProjectCatModal').classList.add('hidden')" class="fixed inset-0 bg-black/60 backdrop-blur-sm"></div>
        ...
        <div class="relative z-10 w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-6">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">Add New Project Category</h3>
                <button onclick="document.getElementById('addProjectCatModal').classList.add('hidden')" type="button" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
            ...
            <form action="{{ route('admin.project-categories.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" required placeholder="e.g. Web Apps, AI Solutions, Mobile Apps" class="w-full px-3.5 py-2.5 text-sm rounded-lg border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                </div>
                ...
                <div class="flex justify-end gap-3">
                    <button onclick="document.getElementById('addProjectCatModal').classList.add('hidden')" type="button" class="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg">
                        Cancel
                    </button>
    ...
    <!-- View Project Category Modals -->
    @foreach($items as $item)
    <div id="viewProjectCatModal-{{ $item->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 text-left font-normal">
        <!-- Backdrop -->
        <div onclick="document.getElementById('viewProjectCatModal-{{ $item->id }}').classList.add('hidden')" class="fixed inset-0 bg-black/60 backdrop-blur-sm"></div>
        ...
        <div class="relative z-10 w-full max-w-lg bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-6 max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center pb-3 border-b border-gray-200 dark:border-gray-700 mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">Project Category Details</h3>
                </div>
                <button onclick="document.getElementById('viewProjectCatModal-{{ $item->id }}').classList.add('hidden')" type="button" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
                    <i data-lucide="x" class="w-5 h-5"></i>
                </button>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="bg-gray-50 dark:bg-gray-700/30 p-4 rounded-xl border border-gray-200/60 dark:border-gray-700/60">
                    ...
                    @else
                        <p class="text-xs text-gray-500 dark:text-gray-400 italic">No projects linked to this category yet.</p>
                    @endif
                </div>
            </div>

            <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                <button onclick="document.getElementById('viewProjectCatModal-{{ $item->id }}').classList.add('hidden')" type="button" class="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg">
                    Close
                </button>
                <button onclick="document.getElementById('viewProjectCatModal-{{ $item->id }}').classList.add('hidden'); document.getElementById('editProjectCatModal-{{ $item->id }}').classList.remove('hidden');" type="button" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg shadow-md shadow-indigo-500/20 inline-flex items-center">
    ...
    <!-- Edit Project Category Modals -->
    @foreach($items as $item)
    <div id="editProjectCatModal-{{ $item->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 text-left font-normal">
        <!-- Backdrop -->
        <div onclick="document.getElementById('editProjectCatModal-{{ $item->id }}').classList.add('hidden')" class="fixed inset-0 bg-black/60 backdrop-blur-sm"></div>
        
        <!-- Modal Dialog -->
        <div class="relative z-10 w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-6">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">Edit Project Category</h3>
                <button onclick="document.getElementById('editProjectCatModal-{{ $item->id }}').classList.add('hidden')" type="button" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
                    <i data-lucide="x" class="w-5 h-5"></i>
                </button>
            </div>
            <form action="{{ route('admin.project-categories.update', $item->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Category Name *</label>
                ...
                <div class="flex justify-end gap-3">
                    <button onclick="document.getElementById('editProjectCatModal-{{ $item->id }}').classList.add('hidden')" type="button" class="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg">
                        Cancel
                    </button>
                    <button type="submit" class="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg shadow-md shadow-indigo-500/20">
                        Update Category
...
<script>
function toggleCatPopover(id) {
    const target = document.getElementById('cat-popover-' + id);
    const isHidden = target.classList.contains('hidden');
    
    // Close all popovers
    document.querySelectorAll('.cat-popover').forEach(el => el.classList.add('hidden'));
